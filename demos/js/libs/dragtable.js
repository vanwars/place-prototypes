...
dragtable = {
  ...
  // The thead business is taken straight from sorttable.
    makeDraggable: function(table) {
        ...
        for (var i = 0; i < headers.length; i++) {
            if (headers[i].classList.contains('no-drag')) {
                continue;
            }
 ...
 absolutePosition: function(elt, stopAtRelative) {
  ...
  dragStart: function(event, id) {
    ...
    // if an active column drag is happening (in column-resizer),
    // then column dragging is disabled.
    if (curCol) {
        console.log('column dragging is temporarily disabled');
        return;
    }
    var el;
    var x, y;
